...
import RailSelected from './src/RailSelected';
import Notch from './src/Notch';
...
const SliderScreen = () => {
  ...
  const renderThumb = useCallback(
    (name) => (
      <Thumb
        ...
        onResponderGrant={() => console.log(`Grant on ${name}`)}
        onResponderMove={() => console.log(`Move on ${name}`)}
        onResponderRelease={() => console.log(`Release on ${name}`)}
      />
    ),
    [],
  );
  ...
  const handleValueChange = useCallback((lowValue, highValue) => {
    console.log(`Values Changed - Low: ${lowValue}, High: ${highValue}`);
    setLow(lowValue);
    setHigh(highValue);
  }, []);
  const toggleRangeEnabled = useCallback(
    () => {
      console.log(`Toggle Range Enabled: ${!rangeDisabled}`);
  ...
  const toggleFloatingLabel = useCallback(
    () => {
      console.log(`Toggle Floating Label: ${!floatingLabel}`);
